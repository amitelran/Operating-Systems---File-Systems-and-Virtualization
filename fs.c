...
/**********************************************************************************************/
/***************************  Assignment 4 - get inodestat data  ******************************/
/*
Free inodes: <free inode number (ref = 0)>
Valid inode: <valid inode number>
Refs per inode: <ratio of total number of refs / number of used inodes>
*/

struct inodeStatData inodeCacheData;



struct inodeStatData getinodeStatData(void)
{
  struct inode *ip;
  inodeCacheData.freeInodes = 0;
  inodeCacheData.validInodes = 0;
  inodeCacheData.totalNumOfRefs = 0;
  inodeCacheData.numOfUsedInodes = 0;

  acquire(&icache.lock);
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++)
  {
    // If number of references if 0 --> free inode
    if (ip->ref == 0)
    {
      inodeCacheData.freeInodes++;
    }
    // Else, increment number of used inodes (at least one ref), and number of refs per inode
    else
    {
      inodeCacheData.numOfUsedInodes++;
      inodeCacheData.totalNumOfRefs += ip->ref;
    }

    if (ip->flags & I_VALID)
    {
      inodeCacheData.validInodes++;
    }
  }
  release(&icache.lock);
  return inodeCacheData;
}
